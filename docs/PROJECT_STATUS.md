...
Stanje: **Faza 0 i Faza 1 ZAVRŠENE** (T1.1–T1.9 gotovi); **Faza 2 u toku — T2.1 ✅** (šema baze povučena u repo: `supabase/migrations/..._remote_schema.sql`, 17 tabela / 53 RLS politike). Sledeće: **T2.2** (TS tipovi `supabase gen types`), **T2.3** (storage bucket-i; RLS već u snapshot-u). Profil konsolidovan na **2 ekrana** (svoj `profile/index.tsx` + tuđi `profile-details/[id]`; `test/[id]` obrisan, navigacija prevezana). App se build-uje i diže na uređaju bez crash-a (vidi §8 dnevnik). ESLint + Prettier prolaze čisto (0 errors); mrtav kod očišćen.
...
- [x] ~~**Šema baze NIJE u repozitorijumu**~~ ✅ **REŠENO (T2.1)** — šema povučena `pg_dump`-om u `supabase/migrations/20260624145146_remote_schema.sql` (17 tabela, 53 RLS politike, 16 funkcija, 7 trigera). Root `.gitignore` više ne ignoriše ceo `supabase/` (samo `.temp`/`.env.local`). Ostaje T2.2 (TS tipovi) + T2.3 (storage bucket-i).
