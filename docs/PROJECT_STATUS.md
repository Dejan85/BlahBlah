...
> Poslednje ažuriranje: **2026-06-24** (T3.5 — `lib/streak.ts`: kalendarski streak + reset + bunny rolling deadline + bonus dani 8/20/28/48)
...
Stanje: **Faza 0, 1 i 2 ZAVRŠENE** (T1.1–T1.9, T2.1–T2.3 gotovi); **Faza 3 u toku — T3.1 ✅, T3.2 ✅, T3.3 ✅, T3.4 ✅, T3.5 ✅** (`lib/formatCount.ts` + `lib/blahScore.ts` — Blah Score formula `(Blahs×4)+(Followers×0.8)+Streak Bonus` + test; DB storage: `profiles.blah_score` + `profiles.blahs_sent` kolone, migracija primenjena live + tipovi regenerisani; UI: svoj profil obračunava+upisuje skor, prikazuje `formatCount(skor)` sa crvenim Blahs statom MyProfile 8.9; **`lib/streak.ts` — streak obračun po kalendarskom danu (timezone-aware) + reset pravila + `currentStreakDay` za skor + rolling bunny deadline (≤3h pre 24h, Home 2.0); bonus dani 8/20/28/48 izvučeni u `streak.ts` kao single source, `blahScore.ts` ih uvozi**); **sledeće: T3.6** (☁️ ⚡ Streak DB + background reset job — Supabase cron/edge; tu se profil `streakDay=0` zamenjuje pravim danom preko `currentStreakDay`). Backup baze kompletan: šema u repo (`supabase/migrations/..._remote_schema.sql`, 17 tabela / 53 RLS), TS tipovi (`types/database.types.ts` kroz `@/types`), storage bucket-i + 10 storage RLS politika (`supabase/storage_buckets_and_policies.sql`). ⚠️ Dva latentna bug-a otkrivena u Fazi 2: (1) kod gađa nepostojeće tabele `friends`/`friend_requests`; (2) `avatars` bucket je private a kod koristi `getPublicUrl` (oba §4). Profil konsolidovan na **2 ekrana** (svoj `profile/index.tsx` + tuđi `profile-details/[id]`; `test/[id]` obrisan, navigacija prevezana). App se build-uje i diže na uređaju bez crash-a (vidi §8 dnevnik). ESLint + Prettier prolaze čisto (0 errors); mrtav kod očišćen.
...
- **Faza 3** — signature mehanike (Blah Score → streak → recovery → ephemeral → premium) — **najveći blok, U TOKU (T3.1–T3.4 ✅ Blah Score kompletan; T3.5 ✅ `lib/streak.ts`; sledeći T3.6 — Streak DB + reset job)**, sve kroz `lib/` + testovi (`ARCHITECTURE.md` §2.5)
...
1. **Blah Score** — temelj, od njega zavise streak/recovery/boost. ✅ formula (`lib/blahScore.ts`, T3.2) + formatiranje (`lib/formatCount.ts`, T3.1) + DB storage (`profiles.blah_score`/`blahs_sent`, T3.3) + UI integracija (svoj profil obračunava/upisuje/prikazuje skor, crveni Blahs stat, T3.4). ⏳ Streak bonus dani aktivni u UI tek kad streak ima DB (T3.6 — profil sad šalje `streakDay=0`; logika streak-a gotova u `lib/streak.ts` T3.5).
2. **Streak + Blah Recovery** — ✅ `lib/streak.ts` (T3.5): kalendarski streak + reset + bonus dani + bunny rolling deadline. ⏳ ostaje: streak DB + reset job (T3.6), recovery 26h/13h + €1.99 (T3.7+)
