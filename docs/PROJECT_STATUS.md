...
> Poslednje ažuriranje: **2026-06-24** (T3.4 — Blah Score integracija u UI: crveni Blahs stat + obračun/upis skora)
...
- **Faza 3** — signature mehanike (Blah Score → streak → recovery → ephemeral → premium) — **najveći blok, U TOKU (T3.1 ✅, T3.2 ✅, T3.3 ✅, T3.4 ✅ — Blah Score kompletan; sledeći T3.5 — streak)**, sve kroz `lib/` + testovi (`ARCHITECTURE.md` §2.5)
...
1. **Blah Score** — temelj, od njega zavise streak/recovery/boost. ✅ formula (`lib/blahScore.ts`, T3.2) + formatiranje (`lib/formatCount.ts`, T3.1) + DB storage (`profiles.blah_score`/`blahs_sent`, T3.3) + UI integracija (svoj profil obračunava/upisuje/prikazuje skor, crveni Blahs stat, T3.4). ⏳ Streak bonus dani aktivni tek kad streak proradi (T3.5/T3.6 — sad `streakDay=0`).
