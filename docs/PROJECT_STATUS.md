...
> Poslednje ažuriranje: **2026-06-24** (T2.2)
...
Stanje: **Faza 0 i Faza 1 ZAVRŠENE** (T1.1–T1.9 gotovi); **Faza 2 u toku — T2.1 ✅ + T2.2 ✅** (šema baze u repo: `supabase/migrations/..._remote_schema.sql`, 17 tabela / 53 RLS politike; TS tipovi generisani `types/database.types.ts`, izloženi kroz `@/types`). Sledeće: **T2.3** (storage bucket-i; RLS već u snapshot-u). ⚠️ T2.2 otkrio latentni bug — kod gađa nepostojeće tabele `friends`/`friend_requests` (§4). Profil konsolidovan na **2 ekrana** (svoj `profile/index.tsx` + tuđi `profile-details/[id]`; `test/[id]` obrisan, navigacija prevezana). App se build-uje i diže na uređaju bez crash-a (vidi §8 dnevnik). ESLint + Prettier prolaze čisto (0 errors); mrtav kod očišćen.
