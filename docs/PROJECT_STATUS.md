...
> Poslednje ažuriranje: **2026-06-24** (T3.3 — `blah_score`/`blahs_sent` kolone na `profiles` + migracija)
...
| **Blah Score / Streak / Recovery** | 8% | ~15% |
...
- **Faza 3** — signature mehanike (Blah Score → streak → recovery → ephemeral → premium) — **najveći blok, U TOKU (T3.1 ✅, T3.2 ✅, T3.3 ✅, sledeći T3.4 — UI integracija)**, sve kroz `lib/` + testovi (`ARCHITECTURE.md` §2.5)
...
1. **Blah Score** — temelj, od njega zavise streak/recovery/boost. ✅ formula (`lib/blahScore.ts`, T3.2) + formatiranje (`lib/formatCount.ts`, T3.1) + DB storage (`profiles.blah_score`/`blahs_sent`, T3.3); ❌ ostaje real-time obračun + UI prikaz (T3.4)
