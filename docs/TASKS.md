...
- [ ] **T2.3** Izvući + commit-ovati RLS politike, dokumentovati buckete — *delom gotovo: 53 RLS politike već u `..._remote_schema.sql` (T2.1). Ostaje: dokumentovati storage **bucket-e** (storage.buckets su data, ne u schema-only dump-u) + eventualno storage RLS.*
