...
- [x] **T3.6** ✅ Streak DB + background reset job. **Kolone** na `profiles` (migracija `20260624170500_streak_columns_and_reset_job.sql`, primenjena live): `streak_day` / `last_blah_at` (timestamptz) / `streak_tz_offset` (minuti, CET=+60). **Reset job = pg_cron + tanak SQL sweep** (`reset_lapsed_streaks()`, zakazan `'0 * * * *'`): nulira streak kad je razmak ≥2 kalendarska dana (tz-aware), atomski single-UPDATE → concurrency-safe. **Odluka (vidi §Odluke):** math OSTAJE u `lib/streak.ts`; SQL sweep je VERAN PORT JEDNE granice (`getStreakStatus`→`'lost'`), **zakovan PINNING testom** u `streak.test.ts` (SQL formula == `isStreakLost` oko granice, UTC+CET+IST). Advance (+1/reset-na-slanje) se NE portuje u SQL — radi ga klijent (`registerBlah`, anti-drift kao T3.3). **Wiring:** (1) `app/blahs/new.tsx` slanje → `registerBlah` + upis `streak_day`/`last_blah_at`/`streak_tz_offset` **+ inkrement `blahs_sent`** (usput: bio nikad inkrementiran → skor nije rastao; A2 real-time increment time zatvoren); (2) `app/profile/index.tsx` → `currentStreakDay(state, now, tz)` umesto `streakDay=0` (pravi bonus 8/20/28/48) + **on-read lazy reset** (backup cronu). Novi `lib/streak.ts` `isStreakLost`. `pg_cron` enable-ovan live preko pooler-a (radilo). `tsc` čist, ESLint 0, **test 53/53**.
